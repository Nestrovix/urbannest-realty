...
├── assets/fonts/           Self-hosted Bricolage Grotesque + Source Serif 4 (woff2)
...
## 5 · Customising
- Colours/typography: top of `css/style.css` (`:root` tokens). Ink is graphite `--ink` (#17181B) on paper `--paper` (#F4F5F7); `--blue` (#2F5BEA) is the single accent for links, actions and markers (use `--blue-lt` for blue text on graphite). Display type is Bricolage Grotesque (`--font-head`), body copy is Source Serif 4 (`--font-body`).
