...
├── assets/fonts/           Self-hosted Schibsted Grotesk + Newsreader (woff2)
...
## 5 · Customising
- Colours/typography: top of `css/style.css` (`:root` tokens). Ink is graphite `--ink` (#17181B) on paper `--paper` (#F4F5F7); `--blue` (#2F5BEA) is the single accent for links, actions and markers (use `--blue-lt` for blue text on graphite). Display type is Schibsted Grotesk (`--font-head`), body copy is Newsreader (`--font-body`).
